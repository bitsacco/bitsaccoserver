...
use ::entity::{
    wallets, wallet_transactions, fedimint_operations,
    sea_orm_active_enums::{WalletStatus, TransactionType, TransactionStatus, ReserveType, FedimintOperationType}
};
use sea_orm::Set;

use crate::repositories::{Repositories, RepositoryError};
use super::fedimint::{FedimintClientService, FedimintOperation, FedimintOperationResult, FedimintError};
...
impl WalletService {
    ...
    pub async fn create_wallet(&self, request: CreateWalletRequest) -> WalletResult<wallets::Model> {
        // Check if user already has an active wallet for this federation
        let existing_wallets = self.repositories.wallets
        ...
        self.repositories.wallet_reserves
            .upsert_reserve(
                created_wallet.id,
    ...
    pub async fn get_balance(&self, wallet_id: Uuid, owner_id: Uuid) -> WalletResult<WalletBalance> {
        let wallet = self.get_wallet_for_owner(wallet_id, owner_id).await?;
        let reserve_summary = self.repositories.wallet_reserves
    ...
    pub async fn deposit(&self, request: DepositRequest) -> WalletResult<DepositResponse> {
        let wallet = self.get_active_wallet_for_owner(request.wallet_id, request.owner_id).await?;
        ...
        let created_transaction = self.repositories.wallet_transactions.create(transaction).await?;
        ...
        let fedimint_operation = fedimint_operations::ActiveModel {
            ...
            fee_msat: Set(None),
            request: Set(serde_json::to_value(&request).ok().map(Into::into)),
            response: Set(None),
            error_details: Set(None),
            retry_count: Set(0),
            last_retry_at: Set(None),
            processed_at: Set(None),
            expires_at: Set(Some((chrono::Utc::now() + chrono::Duration::hours(24)).into())),
            created_at: Set(chrono::Utc::now().into()),
            updated_at: Set(chrono::Utc::now().into()),
            created_by: Set(Some(request.owner_id)),
            updated_by: Set(Some(request.owner_id)),
        ...
        let fedimint_op = self.repositories.fedimint_operations.create(fedimint_operation).await?;
        ...
        let updated_transaction = self.repositories.wallet_transactions.update(transaction).await?;
        ...
        let fedimint_result = self.fedimint
            .execute_operation(wallet.federation_id.as_ref().unwrap_or(&"default".to_string()), deposit_operation)
            .await?;

        let deposit_address = match &fedimint_result {
            FedimintOperationResult::DepositAddress { address, .. } => address.clone(),
            _ => return Err(WalletError::ExternalService(
                "Unexpected response from Fedimint for deposit".to_string()
            )),
        };

        // Update Fedimint operation with response
        self.repositories.fedimint_operations
            .update_response(fedimint_op.id, serde_json::to_value(&fedimint_result).unwrap())
            .await?;
    ...
    pub async fn withdraw(&self, request: WithdrawRequest) -> WalletResult<WithdrawResponse> {
        ...
        let balance = self.get_balance(request.wallet_id, request.owner_id).await?;
        ...
        self.repositories.wallets
            .update_balance(wallet.id, wallet.balance_msat, wallet.pending_in_msat, wallet.pending_out_msat + amount_msat)
            .await?;
        ...
        let fedimint_operation = fedimint_operations::ActiveModel {
            ...
            last_retry_at: Set(None),
            processed_at: Set(None),
            expires_at: Set(Some((chrono::Utc::now() + chrono::Duration::hours(1)).into())),
            created_at: Set(chrono::Utc::now().into()),
            updated_at: Set(chrono::Utc::now().into()),
            created_by: Set(Some(request.owner_id)),
            updated_by: Set(Some(request.owner_id)),
        ...
        let fedimint_result = self.fedimint
            .execute_operation(wallet.federation_id.as_ref().unwrap_or(&"default".to_string()), withdraw_operation)
            .await?;
    ...
    pub async fn generate_invoice(&self, request: InvoiceRequest) -> WalletResult<InvoiceResponse> {
        ...
        if request.amount_msats == 0 {
            return Err(WalletError::InvalidAmount("Amount must be greater than 0".to_string()));
        }
        ...
        let fedimint_operation = fedimint_operations::ActiveModel {
            ...
            last_retry_at: Set(None),
            processed_at: Set(None),
            expires_at: Set(Some((chrono::Utc::now() + chrono::Duration::seconds(request.expiry_secs.unwrap_or(3600) as i64)).into())),
            created_at: Set(chrono::Utc::now().into()),
            updated_at: Set(chrono::Utc::now().into()),
            created_by: Set(Some(request.owner_id)),
            updated_by: Set(Some(request.owner_id)),
        ...
        let fedimint_result = self.fedimint
            .execute_operation(wallet.federation_id.as_ref().unwrap_or(&"default".to_string()), invoice_operation)
            .await?;

        let (invoice, payment_hash) = match &fedimint_result {
            FedimintOperationResult::InvoiceGenerated { invoice, payment_hash, .. } => (invoice.clone(), payment_hash.clone()),
            _ => return Err(WalletError::ExternalService(
        ...
        let final_transaction = self.repositories.wallet_transactions.update(transaction).await?;
        ...
        Ok(InvoiceResponse {
            ...
            payment_hash,
            amount_msats: request.amount_msats,
            expires_at: chrono::Utc::now() + chrono::Duration::seconds(request.expiry_secs.unwrap_or(3600) as i64),
        })
    }
    ...
    pub async fn pay_invoice(&self, request: PayInvoiceRequest) -> WalletResult<PaymentResponse> {
        ...
        let fedimint_operation = fedimint_operations::ActiveModel {
            ...
            amount_msat: Set(if amount_msat > 0 { Some(amount_msat) } else { None }),
            fee_msat: Set(None),
            request: Set(serde_json::to_value(&request).ok().map(Into::into)),
            response: Set(None),
            error_details: Set(None),
            ...
            last_retry_at: Set(None),
            processed_at: Set(None),
            expires_at: Set(Some((chrono::Utc::now() + chrono::Duration::minutes(5)).into())),
            created_at: Set(chrono::Utc::now().into()),
            updated_at: Set(chrono::Utc::now().into()),
            created_by: Set(Some(request.owner_id)),
            updated_by: Set(Some(request.owner_id)),
        ...
        let fedimint_result = self.fedimint
            .execute_operation(wallet.federation_id.as_ref().unwrap_or(&"default".to_string()), payment_operation)
            .await?;

        let (payment_hash, _operation_id) = match &fedimint_result {
            FedimintOperationResult::PaymentSent { payment_hash, operation_id, .. } => (payment_hash.clone(), operation_id.clone()),
            _ => return Err(WalletError::ExternalService(
    ...
    pub async fn get_transactions(
        ...
        let transactions = self.repositories.wallet_transactions
    ...
    pub async fn get_transaction(
        ...
        let transaction = self.repositories.wallet_transactions
            .find_by_id(transaction_id)
            .await?
            .ok_or(WalletError::TransactionNotFound(transaction_id))?;

        // Verify user owns the wallet
        self.get_wallet_for_owner(transaction.wallet_id, owner_id).await?;
    ...
    pub async fn sync_wallet(&self, wallet_id: Uuid, owner_id: Uuid) -> WalletResult<()> {
        let wallet = self.get_active_wallet_for_owner(wallet_id, owner_id).await?;

        let sync_operation = FedimintOperation::SyncWallet;
        let _result = self.fedimint
            .execute_operation(wallet.federation_id.as_ref().unwrap_or(&"default".to_string()), sync_operation)
            .await?;
    ...
    async fn get_wallet_for_owner(&self, wallet_id: Uuid, owner_id: Uuid) -> WalletResult<wallets::Model> {
        let wallet = self.repositories.wallets
    ...
    async fn get_active_wallet_for_owner(&self, wallet_id: Uuid, owner_id: Uuid) -> WalletResult<wallets::Model> {
        let wallet = self.get_wallet_for_owner(wallet_id, owner_id).await?;
